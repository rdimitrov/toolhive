...
// It loads the embedded registry data at creation time for better performance
func NewEmbeddedRegistryProvider() *EmbeddedRegistryProvider {
	data, err := embeddedRegistryFS.ReadFile("data/registry.json")
	if err != nil {
		// If we can't read embedded data, return a provider with nil registry
		// GetRegistry will return the error
		return &EmbeddedRegistryProvider{registry: nil}
	}

	registry, err := parseRegistryData(data)
	if err != nil {
		// If we can't parse embedded data, return a provider with nil registry
		return &EmbeddedRegistryProvider{registry: nil}
	}
	...
	return &EmbeddedRegistryProvider{registry: registry}
}

// GetRegistry returns the pre-loaded registry data
func (p *EmbeddedRegistryProvider) GetRegistry() (*Registry, error) {
	if p.registry == nil {
		return nil, fmt.Errorf("failed to load embedded registry data during provider initialization")
	}
